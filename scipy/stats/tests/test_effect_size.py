...
class TestCohensD:
    ...
    def setup_method(self):
        ...
        # Known result: mean_diff = -1, pooled_std = sqrt(2), d = -1/sqrt(2)
        self.expected_d = -1.0 / np.sqrt(2.0)
    ...
    def test_pooled_vs_unpooled(self):
        d_pooled = cohens_d(self.x1, self.y1, pooled=True)
        d_unpooled = cohens_d(self.x1, self.y1, pooled=False)
        assert not np.allclose(d_pooled, d_unpooled)
        expected_unpooled = (np.mean(self.x1) - np.mean(self.y1)) / np.std(self.x1, ddof=1)
    ...
    def test_broadcasting_compatibility(self):
        ...
        with pytest.raises(ValueError, match="not compatible for broadcasting"):
            cohens_d(x_wrong_shape, y_wrong_shape)
