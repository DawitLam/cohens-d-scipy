...
def test_direct_import():
    ...
    try:
        ...
        print(f"✅ Direct import works: d = {result:.3f}")
        return True
...
def test_installed_import():
    ...
    try:
        ...
        # Quick test
        result = cohens_d(np.array([1, 2, 3]), np.array([2, 3, 4]))
        print(f"✅ Installed package import works: d = {result:.3f}")
        return True
    except Exception as e:
...
if __name__ == "__main__":
    ...
    print(f"\n=== Summary ===")
    print(f"Direct import: {'✅' if results[0] else '❌'}")
    print(f"Installed import: {'✅' if results[1] else '❌'}")  
    print(f"Intensive scripts: {'✅' if results[2] else '❌'}")
    
    if all(results):
        print("\n🎉 All import scenarios work correctly!")
        sys.exit(0)
    else:
        print("\n⚠️ Some import scenarios failed. GitHub Actions may have issues.")
        sys.exit(1)