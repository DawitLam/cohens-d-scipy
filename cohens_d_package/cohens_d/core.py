...
def _handle_nan_policy(x, y, nan_policy, axis, keepdims, ddof):
 """
 Consolidated NaN handling logic for Cohen's d calculation.

 Returns computed statistics based on nan_policy.
 """
 if nan_policy == 'omit':
 ...
 return mean_x, mean_y, var_x, var_y, n_x, n_y
 else:
...
def cohens_d(x, y=None, *, paired=False, bias_correction=False, axis=None,
 ...
 # Enhanced input validation: check for infinite values and non-numeric data
 def _validate_array_finite(arr, name):
 """Validate array contains finite numeric values."""
 if not np.issubdtype(arr.dtype, np.number):
 raise TypeError(f"Input {name} must contain numeric data, got dtype: {arr.dtype}")
 if nan_policy == 'raise':
 ...
 _validate_array_finite(x, 'x')
 if y is not None:
 _validate_array_finite(y, 'y')
 ...
 # Store alternative for potential future use (confidence intervals, etc.)
 ...
 if y is None:
 mean_x, std_x = _handle_nan_policy(x, None, nan_policy, axis, keepdims, ddof)
 ...
 # Use consolidated NaN handling
 mean_x, mean_y, var_x, var_y, n_x, n_y = _handle_nan_policy(
 x, y, nan_policy, axis, keepdims, ddof)

 if pooled:
 # Handle keepdims for sample sizes when needed
 if axis is not None and keepdims and nan_policy != 'omit':
 ...
 # Use first sample's standard deviation
 if nan_policy == 'omit':
 std_pooled = np.nanstd(x, axis=axis, ddof=ddof, keepdims=keepdims)
 else:
 std_pooled = np.std(x, axis=axis, ddof=ddof, keepdims=keepdims)
 ...
 # Future extensibility: Could return a result object with additional statistics
 # class CohensResult:
 # def __init__(self, effect_size, confidence_interval=None, ...):
 # self.effect_size = effect_size
 # self.confidence_interval = confidence_interval
 # ...
 # return CohensResult(d) if return_object else d
